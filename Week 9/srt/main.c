...
// used to store RGB values of pixels
typedef struct
{
  float r;
  float g;
  ...
  long tid = (long)arg;
	...
	int ys;
	    ...
	    for( i=0; i<3; i++)
            ...
            // add pixel to temporary array of new values (so it doesn't affect
	    // the threads currently running with its new value)
            RGB_t temp;
	    temp.r = scaled_color[0];
	    temp.g = scaled_color[1];
	    temp.b = scaled_color[2];
	    toPrint[(px*width)+py] = temp;
        }
    }
    pthread_exit(0);
...
main( int argc, char **argv )
{
    ...
    toPrint = (RGB_t*)malloc(sizeof(RGB_t) * width * height);
    ...
    {
      rs = pthread_create(&threadID[thread], 0, threadFunc, (void*)thread);
      if (rs)
      {
    ...
    {
      rs = pthread_join(threadID[a], NULL);
      if (rs)
      {
    ...
    for (px = 0; px < width; ++px)
    {
      for (py = 0; py < height; ++py)
      {
	RGB_t print = toPrint[px*width+py];
	printf("%.0f %.0f %.0f\n",
